...
const CategoriesList: React.FC = () => {
  ...
  const fetchCategories = useCallback(async () => {
    if (!canView) {
      toast.error('Bạn không có quyền xem categories');
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Không thể tải danh sách categories');
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async (category: Category) => {
    if (!canDelete) {
      toast.error('Bạn không có quyền xóa category');
      return;
    }

    if (!window.confirm(`Bạn có chắc muốn xóa category "${category.name}"?`)) {
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Không thể xóa category');
    }
  };
  ...
  const handleEdit = (category: Category) => {
    if (!canEdit) {
      toast.error('Bạn không có quyền chỉnh sửa category');
      return;
    }
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-lg xl-down:rounded-md shadow-sm border border-gray-200 dark:border-neutral-700 p-4 xl-down:p-3 sm-down:p-2">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="w-full md:w-48">
            <input
              type="number"
              placeholder={t('categories.userId')}
        ...
        <div className="mt-4 xl-down:mt-3 flex items-center gap-4 text-sm xl-down:text-xs text-gray-600 dark:text-gray-400">
          <span>{t('categories.total')}: <strong>{total}</strong> categories</span>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 xl-down:py-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-sm xl-down:text-xs">
              {searchTerm || selectedUserId ? t('categories.tryFilter') : t('categories.noData')}
            </p>
          </div>
        ) : (
          <>
