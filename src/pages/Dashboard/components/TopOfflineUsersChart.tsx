...
interface TopOfflineUsersChartProps {
  data: TopOfflineUser[];
}

const TopOfflineUsersChart: React.FC<TopOfflineUsersChartProps> = ({ data }) => {
  const { t } = useTranslation('dashboard');

  // Transform data for chart
  const chartData = data.map(item => ({
    name: item.username,
    days: item.offlineDays,
    email: item.email
  }));
    ...
    <div className="bg-white dark:bg-neutral-900 rounded-xl xl-down:rounded-lg shadow-sm border border-gray-200 dark:border-neutral-700 p-6 xl-down:p-4 sm-down:p-3">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Bar 
            dataKey="days" 
            fill="#ef4444" 
            name={t('charts.topOffline.offlineDays')}
            radius={[8, 8, 0, 0]}
          />
        </BarChart>
      </ResponsiveContainer>
      ...
      <div className="mt-6 xl-down:mt-4 sm-down:mt-3">
        <h3 className="text-lg xl-down:text-base sm-down:text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3 xl-down:mb-2">
          {t('charts.topOffline.offlineUsersList')}
        </h3>
        <div className="space-y-2 xl-down:space-y-1.5 sm-down:space-y-1 max-h-64 overflow-y-auto">
          {data.slice(0, 5).map((user) => (
            ...
            >
              <div className="w-10 h-10 xl-down:w-8 xl-down:h-8 sm-down:w-7 sm-down:h-7 rounded-full bg-gradient-to-br from-red-400 to-red-600 flex items-center justify-center text-white font-semibold flex-shrink-0">
                {user.avatar ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs xl-down:text-[10px] text-gray-600 dark:text-gray-400 truncate">
                  {user.lastSeenAt ? format(new Date(user.lastSeenAt), 'HH:mm - dd/MM/yyyy') : 'N/A'}
                </p>
              </div>
              <div className="flex-shrink-0">
                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">
                  {user.offlineDays} {user.offlineDays === 1 ? t('charts.topOffline.day') : t('charts.topOffline.days')}
                </span>
              </div>
            </div>
          ))}
...
export default TopOfflineUsersChart;
