...
interface TopNotesCreatorsChartProps {
  data: TopNotesCreator[];
}

const TopNotesCreatorsChart: React.FC<TopNotesCreatorsChartProps> = ({ data }) => {
    ...
    <div className="bg-white dark:bg-neutral-900 rounded-xl xl-down:rounded-lg shadow-sm border border-gray-200 dark:border-neutral-700 p-6 xl-down:p-4 sm-down:p-3">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Bar 
            dataKey="notes" 
            fill="#8b5cf6" 
            name={t('charts.topNotesCreators.notesCount')}
            radius={[8, 8, 0, 0]}
          />
        </BarChart>
      </ResponsiveContainer>
      ...
      <div className="mt-6 xl-down:mt-4 sm-down:mt-3">
        <h3 className="text-lg xl-down:text-base sm-down:text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3 xl-down:mb-2">
          {t('charts.topNotesCreators.topCreatorsList')}
        </h3>
        <div className="space-y-2 xl-down:space-y-1.5 sm-down:space-y-1 max-h-64 overflow-y-auto">
          {data.slice(0, 5).map((user) => (
            ...
            >
              <div className="w-10 h-10 xl-down:w-8 xl-down:h-8 sm-down:w-7 sm-down:h-7 rounded-full bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white font-semibold flex-shrink-0">
                {user.avatar ? (
              ...
              <div className="flex-shrink-0">
                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">
                  {user.notesCount} {user.notesCount === 1 ? t('charts.topNotesCreators.note') : t('charts.topNotesCreators.notes')}
                </span>
              </div>
            </div>
          ))}
...
export default TopNotesCreatorsChart;
