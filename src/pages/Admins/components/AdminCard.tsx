    ...
    <div className="bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700 p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{admin.name}</h3>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(admin.isActive)}`}>
      ...
      <div className="mt-4">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t('permissionsLabel')}</h4>
        <div className="flex flex-wrap gap-1">
          {admin.adminPermissions?.slice(0, 3).map((permission) => (
            <div
              key={permission}
              className="group relative flex items-center px-2 py-1 text-xs bg-gray-100 dark:bg-neutral-700 text-gray-600 dark:text-gray-300 rounded hover:bg-gray-200 dark:hover:bg-neutral-600 transition-colors"
            >
          ...
          {admin.adminPermissions?.length > 3 && (
            <span className="px-2 py-1 bg-gray-100 dark:bg-neutral-700 text-gray-700 dark:text-gray-300 text-xs rounded">
              +{admin.adminPermissions.length - 3}
            </span>
          )}
        </div>
